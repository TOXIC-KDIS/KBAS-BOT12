...
const handler = async (m, { conn, args, usedPrefix }) => {
    ...
    try {
        ...
        if (!text.trim()) {
            await m.reply(`تخيل و انا سأرسل لك ما تخيلته انا استطيع فعلها نعم ! \nمـــثال:\n*.تخيل* girl with hijan  style`);
            return;
        }
        ...
        await conn.sendMessage(m.chat, { image: { url: media }, caption: `` }, { quoted: m });
    } catch (error) {
        console.error(error);
